...
# anon·service_role 클라이언트는 요청과 무관하고 불변이라 모듈 레벨 싱글톤으로 재사용한다
# (요청마다 새 httpx 세션 생성 방지). 쿼리 빌드는 공유 클라이언트 상태를 변형하지 않으므로
# 스레드풀에서 여러 요청이 동시에 써도 안전하다(supabase/postgrest 2.31 소스로 확인).
# ⚠️ 이 두 인스턴스에 절대 .postgrest.auth(token)/헤더 등 요청별 상태를 붙이지 말 것
...
def get_supabase() -> Client:
    global _anon
    if _anon is None:  # 스레드풀 동시 최초 호출 대비 이중검사 락
        with _lock:
            if _anon is None:
