...
export default function PatientRecordsClient({ patientId }: { patientId: string }) {
  ...
  if (isForbidden) {
    return <ForbiddenNotice message="이 환자의 진료기록에 접근할 권한이 없습니다." />
  }
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          <Link href="/patients" className={styles.back}>← 담당 환자 목록</Link>
      ...
      {isLoading ? (
        ...
        <div className={styles.empty}>
          ...
          <Link href={`/patients/${patientId}/records/new`} className={styles.btnNew}>
            진료기록 작성
          </Link>
        </div>
      ) : (
            ...
            <tbody>
              {records.map((r) => (
                <tr
                  ...
                  className={styles.tr}
                  onClick={() =>
                    router.push(`/records/${r.id}?patient=${patientId}`)
                  }
                >
